...
def prepare():
    ...
    # regardless of weekday. (Keep the guard only for when no carousel.json
    # exists, i.e. stale carousel from a previous run.)

    # Fallback: if the bundle has no carousel paths but carousel.json
    # exists on disk (e.g. generated by generate_carousel_from_bundle.py),
    # load it so we don't delete freshly-generated slides.
    if not carousel_paths:
        carousel_json = os.path.join(state_dir, "carousel.json")
        if os.path.exists(carousel_json):
            # carousel.json exists on disk — load it instead of deleting
            try:
                with open(carousel_json, "r", encoding="utf-8") as f:
                    raw = json.load(f)
                if isinstance(raw, dict) and raw.get("slides"):
                    carousel_paths = [s.get("path", "") for s in raw.get("slides", []) if isinstance(s, dict)]
                    print(f"↩️ Loaded {len(carousel_paths)} carousel slides from existing carousel.json (bundle has no carousel field)")
                elif isinstance(raw, list) and raw:
                    carousel_paths = [p for p in raw if isinstance(p, str)]
                    print(f"↩️ Loaded {len(carousel_paths)} carousel slides from existing carousel.json (list format)")
                else:
                    # Invalid/corrupt carousel.json — remove it
                    os.remove(carousel_json)
                    print(f"Removed invalid carousel.json for non-carousel bundle {active.get('post_id')}")
            except Exception as e:
                print(f"⚠ Could not read carousel.json: {e} — removing it")
                if os.path.exists(carousel_json):
                    os.remove(carousel_json)
        else:
