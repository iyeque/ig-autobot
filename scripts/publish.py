...
def publish_single(user_id, image_path, caption, access_token):
    ...
    if os.path.exists(local_path):
        ...
        binary_payload["image_url"] = hosted_url or "https://iyeque.github.io/ig-autobot/" + local_path.replace("\\", "/")
...
def main():
    ...
    if fmt == "carousel" and not is_carousel and os.path.exists("carousel.json"):
        ...
        # Only fall back to carousel from carousel.json if the carousel
        # workflow explicitly set preferred_fmt = "carousel" (via
        # instagram_format.txt). The main daily poster should not hijack
        # carousels generated by the separate IG Carousel Poster workflow
        # — those are for A/B testing on Mon/Wed/Fri only.
        if preferred_fmt == "carousel" and os.path.exists("carousel.json"):
            hosted = _load_hosted_carousel_urls(".")
            if hosted:
                image_urls = hosted
            else:
